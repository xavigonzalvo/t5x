...
def create_sharded_array(arr, global_shape, global_mesh, mesh_axes):
  ...
  if jax.config.jax_array:
    ...
    return GlobalDeviceArray.from_callback(
        global_shape, global_mesh, mesh_axes, cb
    )
...
class CheckpointsTest(parameterized.TestCase):
  ...
  def test_restore_old_format_as_type(self, dtype):
    ...
    step = np.int32(12)
    old_train_state = make_train_state_non_gda(
        global_mesh,
        global_input_shape,
        mesh_axes,
        step=step,
    ...
    self.save(old_checkpointer, old_train_state)

    new_train_state = make_train_state(
        global_mesh, global_input_shape, mesh_axes, step=step, dtype=dtype
    )
    ...
    restored_train_state = self.restore(new_checkpointer, step=step)

    assert_tree_equal(self, new_train_state, restored_train_state)
  ...
  def test_assignment_map_and_fallback(self, save_fn=None, restore_fn=None):
    ...
    original_train_state = self.pytree_as_gda(
        original_train_state, global_mesh, global_input_shape, mesh_axes
    )
    checkpointer = checkpoints.Checkpointer(
        original_train_state,
        FakePartitioner(global_mesh, mesh_axes),
        self.tmp_dir,
        use_gda=True,
    )
    ...
    train_state = self.pytree_as_gda(
        train_state, global_mesh, global_input_shape, mesh_axes
    )

    checkpointer = checkpoints.Checkpointer(
        _train_state_shapes(train_state),
        FakePartitioner(global_mesh, mesh_axes),
        self.tmp_dir,
    ...
    fallback_state = self.pytree_as_gda(
        fallback_state, global_mesh, global_input_shape, mesh_axes
    )
    ...
    self.assertEqual(
        restored_train_state._optimizer.optimizer_def,
        train_state._optimizer.optimizer_def,
    )
    self.assertEqual(restored_train_state.step, 1337)  # not 42, as saved.
    assert_tree_equal(
        self, train_state.param_states, restored_train_state.param_states
    )
    assert_tree_equal(self, train_state.params, restored_train_state.params)

  def test_assignment_map_and_fallback_noop(self):
    global_mesh = test_utils.create_global_mesh((4, 2), ('x', 'y'))
    mesh_axes = P('x', 'y')
    global_input_shape = (8, 2)
    step = np.int32(10)
    train_state = make_train_state(
        global_mesh, global_input_shape, mesh_axes, step=10
    )
    ...
    self.save(checkpointer, train_state)
    # Add state_transformation_fns, but this should be a no-op since
    # `is_resuming` is True.
    restored_train_state = self.restore(
        checkpointer,
        step=step,
  ...
  def test_fallback_only(self):
    ...
    bias = np.ones(global_input_shape) * 5
    kernel = np.arange(16).reshape(global_input_shape) * 10
    fallback_bias = np.ones(global_input_shape)
    fallback_kernel = np.arange(16).reshape(global_input_shape)

    original_train_state = train_state_lib.FlaxOptimTrainState(
    ...
    self.save(checkpointer, original_train_state)
    ...
    fallback_state = train_state_lib.FlaxOptimTrainState(
        optimizers.Optimizer(
            optimizers.sgd(0.1),
            state=optimizers.OptimizerState(
                step=np.int32(50),
    ...
    fallback_state = fallback_state.state_dict()
    ...
    self.assertEqual(restored_train_state.step, 42)
    assert_tree_equal(
        self, train_state.param_states, restored_train_state.param_states
    )
    assert_tree_equal(self, train_state.params, restored_train_state.params)
...
class OrbaxCheckpointsTest(CheckpointsTest):
  ...
  def checkpoint_manager(
      self, checkpointer, save_dtype=None, restore_dtype=None
  ):
  ...
  def restore(
    ...
    return self.checkpoint_manager(checkpointer, restore_dtype=dtype).restore(
        step=step,
        path=path,
        lazy_parameters=lazy_parameters,
        state_transformation_fns=state_transformation_fns,
  ...
  def test_restore_old_format_as_type(self, dtype):
    ...
    step = np.int32(12)
    old_train_state = make_train_state_non_gda(
        global_mesh, global_input_shape, mesh_axes, step=step, shard=True
    )
    old_checkpointer = checkpoints.Checkpointer(
        old_train_state,
        FakePartitioner(global_mesh, mesh_axes),
    ...
    restored_train_state = self.restore(
        new_checkpointer, step=step, dtype=dtype
    )

    assert_tree_equal(self, new_train_state, restored_train_state)

  def test_restore_old_format(self):
    global_mesh = test_utils.create_global_mesh((2, 2), ('x', 'y'))
    mesh_axes = P('x', 'y')
    global_input_shape = (2, 2)
  ...
  def test_end_to_end(self):
    ...
    restore_cfg = utils.RestoreCheckpointConfig(
        path='', restore_dataset=False  # Path not used in this test.
    )
    ...
    for step in range(init_step + period, max_step + 1):
      ...
      self.assertEqual(
          manager.latest_step(), step if step % period == 0 else step - 1
      )
  ...
  def test_cleanup(self):
    ...
    with mock.patch.object(
        checkpoints.CheckpointManager, '_add_checkpoint_info', autospec=True
    ) as commit_callback:
      ...
      tmp_checkpoint_pattern = (
          'checkpoint_*' + orbax.checkpoint.utils.TMP_DIR_SUFFIX + '*'
      )
      self.assertNotEmpty(
  ...
  def test_restore_without_msgpack(self):
    global_mesh = test_utils.create_global_mesh((4, 2), ('x', 'y'))
    mesh_axes = P('x', 'y')
    global_input_shape = (8, 2)

    train_state = make_train_state(global_mesh, global_input_shape, mesh_axes)
    step = np.int32(train_state.step)
    ...
    ckpt_path = checkpoints.get_checkpoint_dir(self.tmp_dir, step)
    msgpack_path = os.path.join(ckpt_path, 'checkpoint')
    self.assertTrue(os.path.exists(msgpack_path))
    os.unlink(msgpack_path)  # delete msgpack file
    orbax.checkpoint.PyTreeCheckpointHandler().save(
        epath.Path(ckpt_path), {'state': {'step': step}}
    )
    self.assertTrue(os.path.exists(msgpack_path))
    os.unlink(msgpack_path)  # delete msgpack file
    restored = checkpoint_manager.restore(path=ckpt_path)
    # Cheat to replace step. Technically step should be saved using Tensorstore,
    # otherwise it won't be present outside the msgpack and could not be
    # restored.
    restored = restored.replace_step(step)
